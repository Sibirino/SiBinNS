...
void drawSplash() {    
    ...
    if(gfx_con.mute && fopen("/SiBin/splash.bmp", "rb") != 0) {
...
void firmware() {
    ...
    //Chainload SiBin if applicable
    if(EMC(EMC_SCRATCH0) != 69 && EMC(EMC_SCRATCH0) != 67 && fopen("/SiBin.bin", "rb")) {
        size_t size = fsize();
    ...
    if(btn_read() & BTN_VOL_UP){
        if(fopen("/SiBin/Recovery.bin", "rb") != 0) {
            fread((void*)PAYLOAD_ADDR, fsize(), 1);
            fclose();
            if(!fopen("/SiBin.bin", "rb")) {
