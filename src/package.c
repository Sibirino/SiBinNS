...
pkg2_hdr_t *unpackFirmwarePackage(u8 *data, u8 fwVer) {
    ...
    if (hdr->magic != PKG2_MAGIC) {
        error("Package2 Magic invalid!\nThere is a good chance your SiBin build is outdated\nPlease get the newest build from our guide (SiBin.guide) or our discord (discord.reiswitched.team)\nMake sure you replace the SiBin.bin file on your SD card root too\n");
        return NULL;
    }
...
void pkg1_unpack(pk11_offs *offs, u32 pkg1Off) {
    ...
    for (u32 i = 0; i < 3; i++) {
        if (offs->sec_map[i] == 0 && offs->warmboot_base) {
            size_t extSize = 0;
            u8 *extWb = LoadExtFile("/SiBin/warmboot.bin", &extSize);
            if(extWb == NULL) extWb = LoadExtFile("/SiBin/lp0fw.bin", &extSize);
        ...
        if (offs->sec_map[i] == 2 && offs->secmon_base) {
            size_t extSize = 0;
            u8 *extSec = LoadExtFile("/SiBin/secmon.bin", &extSize);
            if(extSec == NULL) extSec = LoadExtFile("/SiBin/exosphere.bin", &extSize);
...
bool hasCustomWb() {
    if (customWb)
        return customWb;
    if(fopen("/SiBin/warmboot.bin", "rb") != 0) {
        customWb = true;
        fclose();
    }
    if(fopen("/SiBin/lp0fw.bin", "rb") != 0) {
        customWb = true;
        fclose();
    }
    return customWb;
...
bool hasCustomSecmon() {
    if (customSecmon)
        return customSecmon;
    if(fopen("/SiBin/secmon.bin", "rb") != 0) {
        customSecmon = true;
        fclose();
    }
    if(fopen("/SiBin/exosphere.bin", "rb") != 0) {
        customSecmon = true;
        fclose();
    }
    return customSecmon;
...
bool hasCustomKern() {
    if (customKernel)
        return customKernel;
    if(fopen("/SiBin/kernel.bin", "rb") != 0) {
        customKernel = true;
        fclose();
    }
    return customKernel;
...
void buildFirmwarePackage(u8 *kernel, u32 kernel_size, link_t *kips_info, pk11_offs *pk11Offs) {
    ...
    size_t extSize = 0;
    u8 *extKern = LoadExtFile("/SiBin/kernel.bin", &extSize);
